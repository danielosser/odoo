...
class SaleOrder(models.Model):
    ...
    def _is_global_discount_already_applied(self):
        ...
        return any(reward.reward_type == 'discount' and\
                   ...
                   for reward in self.order_line.reward_id)
    ...
    def _get_reward_values_discount(self, reward, coupon, **kwargs):
        ...
        if reward.discount_mode == 'per_point':
            max_discount = min(max_discount, reward.discount * self._get_real_points_for_coupon(coupon))
        ...
        if reward.discount_mode in ('per_point', 'per_order'):
            ...
            consumed_points = reward.required_points if not reward.clear_wallet else self._get_real_points_for_coupon(coupon)
    ...
    def _check_update_applied_rewards(self, block=False):
        ...
        point_entry_per_program = {pe.coupon_id.program_id for pe in self.coupon_point_ids}
        ...
        for line in self.order_line:
            ...
            points = self._get_real_points_for_coupon(line.coupon_id) +\
                sum(concerned_lines.mapped('points_cost')) if not line.points_cost else\
                line.points_cost
            ...
            if points < line.reward_id.required_points:
                ...
                values_list = self._get_reward_line_values(line.reward_id, line.coupon_id)
    ...
    def _program_check_compute_points(self, program):
        ...
        # Account for free products that are already given on the sale order if the program is applicable to the current order
        # if program.applies_on in ('both', 'current'):
        #     # Imagine 2 '2 + 1 free' promotion at the same time on a product, we may want both promotion to be
        #     #  obtainable only if 4 of that product are present
        #     applied_rewards = self.env['loyalty.reward'].search(
        #         [('reward_type', '=', 'product'), ('discount_line_product_id', 'in', reward_lines.product_id)])
        #     for reward in applied_rewards:
        #         products_qties[reward.reward_product_id] -= reward.reward_product_qty
        #     # Special case when the program is applicable in the future aswell, we should not block if qties are not matched due to rewards
        #     # This is a weird case where we apply the program to the order (for points) but not always the reward
        #     if program.applies_on == 'both':
        #         products_qties_no_reward = products_qties.copy()
        #     # We need to exclude this program's reward quantities, however in the case of multiple rewards we do not
        #     #  currently know which one will be chosen (if forced_reward is not given), in this case assume the first reward.
        #     # Multiple rewards only make sense in loyalty programs
        #     reward = forced_reward or program.reward_ids.filtered(lambda r: r.reward_type == 'product')[:1]
        #     if reward and reward.reward_type == 'product' and reward not in applied_rewards:
        #         products_qties[reward.reward_product_id] -= reward.reward_product_qty
        ...
        for rule in program.rule_ids:
            ...
            if ordered_rule_products_qty < rule.minimum_qty:
                continue
            # elif program.applies_on == 'both':
            #     # Check if rule can be used to add points for a future order
            #     # If a program has a minimum qty of 2, and a 2+1 free plan, count the points for 2 for a future order
            #     # Can make sense on a loyalty card -> every X product you get one free but you only want it for a next order
            #     ordered_rule_products_qty = sum(products_qties_no_reward[product] for product in rule_products)
            #     if ordered_rule_products_qty < rule.minimum_qty:
            #         continue
    ...
    def _try_apply_program(self, program, coupon=None):
        ...
        Returns a dict containing the error message or containing the associated coupon.
        """
        self.ensure_one()
        # Basic checks
        if program.is_global_discount and self._is_global_discount_already_applied():
        ...
        # NOTE: A code from a coupon gives a reward regardless of program applicability
        if not coupon:
            ...
            if program.applies_on in ('future', 'current') or not coupon:
                coupon = self.env['loyalty.card'].create({
                    ...
                    'shared_code': True,
                    'code': program.code or self.env['loyalty.card']._generate_code(), # TODO: We may never want to use the program's code
