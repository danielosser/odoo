...
class IrRule(models.Model):
    ...
    def clear_cache(self):
        warnings.warn("Deprecated IrRule.clear_cache(), use IrRule.clear_caches() and instead", DeprecationWarning)
        self.clear_caches()
...
{resolution_info}"""
        ...
        company_issue = any('company_id' in (r.domain_force or []) for r in rules)

        def get_record_description(rec):
            # If the user has the access to the company of the record, add this
            # information in the description to help him to change company efficiently
            if company_issue and getattr(rec, 'company_id', False) and rec.company_id in self.env.user.company_ids:
