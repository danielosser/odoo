...
class AccountPayment(models.Model):
    ...
    def _compute_amount_available_for_refund(self):
        for payment in self:
            # Refunds should not be able to be refunded
            if payment.payment_transaction_id.operation == 'refund':
                payment.amount_available_for_refund = 0
            elif payment.payment_transaction_id.sudo().acquirer_id.support_refund:
