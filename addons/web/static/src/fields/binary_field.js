...
class BinaryUploader extends Component {
    setup() {
        this.notification = useService("notification");
        this.id = `o_fileupload_${++BinaryUploader.nextId}`;
        this.state = useState({
            isUploading: false,
        });
...
BinaryUploader.template = "web.BinaryUploader";
BinaryUploader.props = {
    onUploaded: Function,
    onRemove: Function,
    ...
    get url() {
        if (this.state.isValid && this.props.value) {
            if (isBinarySize(this.props.value)) {
                const previewFieldName = this.props.options.preview_binary || this.props.name;
                return url("/web/binary", {
                    model: this.props.record.resModel,
                    id: this.props.record.resId,
                    field: previewFieldName,
                });
            } else {
                // Use magic-word technique for detecting binary type
                const magic = fileTypeMagicWordMap[this.props.value[0]] || "png";
                return `data:binary/${magic};base64,${this.props.value}`;
            }
        }
        return placeholder;
    }
