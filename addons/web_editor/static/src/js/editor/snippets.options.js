...
const Many2oneUserValueWidget = SelectUserValueWidget.extend({
    ...
    async _search(needle) {
        ...
        records.forEach(record => {
            if (record.hasOwnProperty('display_name')) {
                this.displayNameCache[record.id] = record.display_name;
            }
        });

        const hasMore = !!records[this.options.limit];
...
registry.many2one = SnippetOptionWidget.extend({
    display_fields: {
        // Currently unused, only display_name is displayed, name is used when
        // rendering into the page. Not sure about how to pass-in a way to format
        // the displayed record in the select. Maybe they can pass a template id?
        // Not sure how useful it is to keep since we now have working preview.
        'res.partner': ['name', 'display_name', 'city', 'country_id'],
    },
    ...
    async willStart() {
        ...
        this.fields = this.display_fields[oeMany2oneModel] || ['name', 'display_name'];
        this.initialRecord = (await this._rpc({
    ...
    async changeRecord(previewMode, widgetValue, params) {
        ...
        if (previewMode === 'reset') {
            ...
            this.$target.html(this.prevHtml);
        ...
        if (previewMode === true) {
            ...
            this.prevHtml = this.$target.html();
     ...
     * @private
     */
    async _rerenderContacts(contactId, defaultHTML) {
