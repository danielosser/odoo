...
class AccountTax(models.Model):
    ...
    @api.model
    def _get_line_field_value(self, line, field_name, value, relational_model=None, default_value=None):
        if value is not None:
            return value
        if field_name in line._fields:
            return line[field_name]
        return self.env[relational_model] if relational_model else default_value
    ...
    def _prepare_base_line(self, base_line,
        ...
        return {
            'record': base_line,
            'partner': self._get_line_field_value(
                base_line, 'partner_id', partner, relational_model='res.partner'),
            'currency': self._get_line_field_value(
                base_line, 'currency_id', currency, relational_model='res.currency'),
            'product': self._get_line_field_value(
                base_line, 'product_id', product, relational_model='product.product'),
            'taxes': self._get_line_field_value(
                base_line, 'tax_ids', taxes, relational_model='account.tax'),
            'price_unit': self._get_line_field_value(
                base_line, 'price_unit', price_unit, default_value=0.0),
            'quantity': self._get_line_field_value(
                base_line, 'quantity', quantity, default_value=1.0),
            'discount': self._get_line_field_value(
                base_line, 'discount', discount, default_value=0.0),
            'account': self._get_line_field_value(
                base_line, 'account_id', account, relational_model='account.account'),
            'analytic_account': self._get_line_field_value(
                base_line, 'analytic_account_id', analytic_account, relational_model='account.analytic.account'),
            'analytic_tags': self._get_line_field_value(
                base_line, 'analytic_tag_ids', analytic_tags, relational_model='account.analytic.tag'),
            'price_subtotal': self._get_line_field_value(
                base_line, 'price_subtotal', price_subtotal, default_value=0.0),
            'is_refund': is_refund,
        }
    ...
    def _prepare_tax_line(self, tax_line,
        ...
        return {
            'record': tax_line,
            'partner': self._get_line_field_value(
                tax_line, 'partner_id', partner, relational_model='res.partner'),
            'currency': self._get_line_field_value(
                tax_line, 'currency_id', currency, relational_model='res.currency'),
            'taxes': self._get_line_field_value(
                tax_line, 'tax_ids', taxes, relational_model='account.tax'),
            'tax_tags': self._get_line_field_value(
                tax_line, 'tax_tags_ids', tax_tags, relational_model='account.account.tag'),
            'tax_repartition_line': self._get_line_field_value(
                tax_line, 'tax_repartition_line_id', tax_repartition_line, relational_model='account.tax.repartition.line'),
            'group_tax': self._get_line_field_value(
                tax_line, 'group_tax_id', group_tax, relational_model='account.tax'),
            'account': self._get_line_field_value(
                tax_line, 'account_id', account, relational_model='account.account'),
            'analytic_account': self._get_line_field_value(
                tax_line, 'analytic_account_id', analytic_account, relational_model='account.analytic.account'),
            'analytic_tags': self._get_line_field_value(
                tax_line, 'analytic_tags_ids', analytic_tags, relational_model='account.analytic.tag'),
