...
class MailComposer(models.TransientModel):
    ...
    body = fields.Html(
        ...
        render_engine='qweb', sanitize_style=True)
    ...
    def _compute_subject(self):
        for composer in self:
            if composer.template_id.subject:
                ...
                subject = composer.parent_id.subject if composer.parent_id else False
                if not subject:
                    subject = composer.record_name if composer.record_name else False
                if subject:
                    re_prefix = _('Re:')
    ...
    def _compute_attachment_ids(self):
        ...
        for composer in self:
            if composer.template_id and composer.composition_mode == 'mass_mail' and composer.template_id.attachment_ids:
                ...
                rendered_values = composer.template_id._generate_attachments(
                    [composer.res_id],
    ...
    def _compute_partner_ids(self):
        ...
        for composer in self:
            if composer.template_id and composer.composition_mode == 'comment':
                rendered_values = composer.template_id._generate_recipients(
                    [composer.res_id], ['email_to', 'partner_to', 'email_cc'],
                    partners_only=True
                )[composer.res_id]
    ...
    def render_message(self, res_ids):
        ...
        if self.template_id:
            template_values = self.generate_email_for_composer(
                ...
                ['email_to', 'partner_to', 'email_cc', 'attachments', 'attachment_ids', 'mail_server_id'])
        else:
            template_values = {}
        ...
        if self.template_id and (self.template_id[template_fname] or force_void):
            if self.composition_mode == 'comment':
                self[composer_fname] = self.template_id._render_field(
                    ...
                    compute_lang=True,
                    post_process=(template_fname == 'body_html')
                )[self.res_id]
