...
class AccountEdiFormat(models.Model):
    ...
    def _is_embedding_to_invoice_pdf_needed(self, invoice):
        # OVERRIDE
        self.ensure_one()
        return True if self.code == 'fattura_pa' else super()._is_embedding_to_invoice_pdf_needed(invoice)
    ...
    def _create_invoice_from_xml_tree(self, journal, filename, tree):
        ...
        if self._is_fattura_pa(filename, tree):
            return self._import_fattura_pa(tree, self.env['account.move'].with_company(journal.company_id))
