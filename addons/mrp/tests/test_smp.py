...
class TestMrpSerialMassProduce(TestMrpCommon):
    ...
    def test_smp_produce_complex(self):
        ...
        count = mo.product_qty
        # Make some stock and reserve
        for i in range(2):  # 2 lots of 3 to satisfy the need and check lot splitting
        ...
        for i in range(3):  # 3 serial numbers
            lot = self.env['stock.production.lot'].create({
...
class TestMrpSerialMassProduceTimes(common.TransactionCase):
