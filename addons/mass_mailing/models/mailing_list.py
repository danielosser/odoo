...
class MassMailingList(models.Model):
    ...
    def action_merge(self, src_lists):
        ...
        join_select = ', '.join("%s" % "sub_" + field for field in select_values.split(', '))
        insert_query = f"""
        ...
        """
        self.env.cr.execute(insert_query, ({'id': self.id, 'dest_ids':tuple(self.ids), 'ids': tuple(src_lists.ids)}))
