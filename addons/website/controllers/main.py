...
class Website(Home):
    ...
    def _get_customize_views(self, xml_ids):
        self._get_customize_data(self, xml_ids, "ir.ui.view")

    def _get_customize_data(self, keys, model):
    ...
    def theme_customize_get(self, keys, model):
        Data = self._get_customize_data(keys, model)
        return Data.filtered('active').mapped('key')

    @http.route(['/website/theme_customize'], type='json', auth='user', website=True)
    def theme_customize(self, model, enable=None, disable=None, reset_view_arch=False):
        """
        ...
        """
        disabled_data = self._get_customize_data(disable, model).filtered('active')
        if reset_view_arch:
            disabled_data.reset_arch(mode='hard')
        disabled_data.write({'active': False})
        self._get_customize_data(enable, model).filtered(lambda x: not x.active).write({'active': True})
