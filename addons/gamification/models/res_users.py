...
class Users(models.Model):
    ...
    def create(self, values_list):
        ...
        karma_trackings = [
            tracking.create_tracking(
                user.id, 0, vals.get('karma', 0), reason=_('User Creation'))
            for user, vals in zip(res, values_list)
            if vals.get('karma')
        ]
        if karma_trackings:
            tracking.sudo().create(karma_trackings)
    ...
    def _add_karma(self, karma, source=None, reason=''):
        ...
        if not karma:
            return
        tracking = self.env['gamification.karma.tracking']
        tracking.sudo().create([
            tracking.create_tracking(
                user.id, user.karma, user.karma + karma, source, reason)
            for user in self
        ])
        return True
