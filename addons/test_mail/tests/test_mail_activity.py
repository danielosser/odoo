...
class TestORM(TestActivityCommon):
    ...
    def test_week_grouping(self):
        ...
        self.env['mail.test.activity'].create({'date': sunday2, 'name': "Things we said today"}).activity_schedule(
            'test_mail.mail_act_test_todo',
            summary="Make another test asap",
            date_deadline=fields.Date.context_today(MailTestActivityCtx)
        )
        self.env['mail.test.activity'].create({'date': sunday3, 'name': "Tomorrow Never Knows"}).activity_schedule(
            'test_mail.mail_act_test_todo',
