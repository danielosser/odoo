 ...
 * @param {jQuery} $input
 */
function _autocompleteWithPages(self, $input, options) {
    ...
    $input.urlcomplete({
        source: function (request, response) {
            if (request.term[0] === '#') {
                _loadAnchors(request.term).then(function (anchors) {
                    response(anchors);
                });
...
return {
    ...
    autocompleteWithPages: _autocompleteWithPages,
    loadAnchors: _loadAnchors,
};
});
