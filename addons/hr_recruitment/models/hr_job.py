...
class Job(models.Model):
    ...
    def search_panel_select_range(self, field_name):
        if field_name == 'user_id':
            users = self.env['hr.job'].search([('user_id', '!=', None)]).mapped('user_id')
            available_users = self.env['res.users'].search([('id', 'in', users.ids)], limit=SEARCH_PANEL_LIMIT).read(['display_name'])
            return {
                'parent_field': False,
