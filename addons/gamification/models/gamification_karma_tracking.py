...
class KarmaTracking(models.Model):
    ...
    def _get_selection_origin(self):
        return [('res.users', 'User (Manual change)')]
    ...
    reason = fields.Text(default=lambda self: _("Added Manually"), string='Reason')
    origin_ref = fields.Reference(
        string='Source',
        selection=lambda self: self._get_selection_origin(),
        default=lambda self: 'res.users,%s' % self.env.user.id,
    )
    ...
    @api.model
    def create_tracking(self, user_id, old_karma, new_karma, origin=None, reason=''):
        return {
            'user_id': user_id,
            'old_value': old_karma,
            'new_value': new_karma,
            'origin_ref': f'{origin._name},{origin.id}' if origin else f'res.users,{self.env.uid}',
    ...
    def _process_consolidate(self, from_date):
        ...
        to_date = date_utils.end_of(date_utils.end_of(from_date, 'month'), 'day')
