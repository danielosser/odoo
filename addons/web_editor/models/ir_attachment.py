...
class IrAttachment(models.Model):
    ...
    filetype = fields.Char(compute='_compute_filetype', default='image')
    ...
    def _compute_filetype(self):
        for attachment in self:
            attachment.filetype = 'video' if attachment.mimetype in SUPPORTED_VIDEO_MIMETYPES else 'image'
    ...
    def _get_media_info(self):
        ...
        self.ensure_one()
        return self._read_format(['id', 'name', 'description', 'mimetype', 'checksum', 'url', 'type', 'res_id', 'res_model', 'public', 'access_token', 'src', 'width', 'height', 'original_id'])[0]
