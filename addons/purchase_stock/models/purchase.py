...
class PurchaseOrder(models.Model):
    ...
    receipt_status = fields.Selection([
        ...
        ('full', 'Fully Received'),
    ], string='Receipt Status', compute='_compute_receipt_status', store=True, readonly=False)
    ...
    def _compute_receipt_status(self):
        for order in self:
            if order.state in ('done', 'cancel') or not order.picking_ids or all(p.state == 'cancel' for p in order.picking_ids):
