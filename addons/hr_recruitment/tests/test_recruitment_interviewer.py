...
class TestRecruitmentInterviewer(MailCommon):
    ...
    def test_interviewer_group(self):
        ...
        applicant = self.env['hr.applicant'].create({
            ...
            'partner_name': 'toto',
            'job_id': self.job.id,
            'interviewer_ids': [(6, 0, self.simple_user.ids)]
        })
        self.assertTrue(interviewer_group.id in self.simple_user.groups_id.ids, "Simple User should be added as interviewer")

        applicant.write({'interviewer_ids': [(5, 0, 0)]})
        self.assertFalse(interviewer_group.id in self.simple_user.groups_id.ids, "Simple User should be removed from interviewer")
    ...
    def test_interviewer_group_from_job(self):
        ...
        self.assertEqual(self.job.interviewer_ids, applicant.interviewer_ids, "Interviewers should be the same as the job's")

        other_applicant = self.env['hr.applicant'].create({
            'name': 'toto',
            'partner_name': 'toto',
            'job_id': self.job.id,
            'interviewer_ids': [(6, 0, self.manager_user.ids)]
        })
        self.assertEqual(other_applicant.interviewer_ids, self.manager_user)
        self.assertNotEqual(self.job.interviewer_ids, other_applicant.interviewer_ids, "Interviewers should not be the same as the job's")

        # Changing the interviewers on a Job should propagate the change to the Applicants UNLESS
        # they have different interviewers than originally on the job
        self.job.interviewer_ids = (self.interviewer_user | self.simple_user).ids
        self.assertEqual(self.job.interviewer_ids, applicant.interviewer_ids, "Interviewers should be the same as the job's")
        self.assertEqual(applicant.interviewer_ids, (self.interviewer_user | self.simple_user), "Interviewers should be the same as the job's")
        self.assertEqual(other_applicant.interviewer_ids, self.manager_user, "Interviewers should not change")
    ...
    def test_interviewer_access_rights(self):
        ...
        applicant = self.env['hr.applicant'].create({
            ...
            'partner_name': 'toto',
            'job_id': self.job.id,
            'interviewer_ids': [(6, 0, self.interviewer_user.ids)]
        })
        applicant.with_user(self.interviewer_user).read()
        ...
        self.assertEqual(self.job.interviewer_ids, applicant.interviewer_ids)
        applicant.with_user(self.interviewer_user).read()

        # An interviewer can't change the interviewers
        applicant.with_user(self.interviewer_user).interviewer_ids = self.simple_user.ids
        self.assertTrue(self.simple_user.id not in applicant.interviewer_ids.ids)
